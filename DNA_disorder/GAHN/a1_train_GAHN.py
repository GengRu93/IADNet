...
def train(args):
    ...
    #data = data * (1 + 0.05 * np.random.rand(data.shape[0], data.shape[1]))
    #print(data.shape)  #(10000, 64)
    label = dftarget.values
    dxdt = torch.Tensor(label[:,index])
    ...
    x = torch.tensor(data[:, index], requires_grad=True, dtype=torch.float32)
