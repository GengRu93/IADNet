...
def Hqp(valpair1,valpair2):
    ...
    return np.array([p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,
                     K * (q2 + q6 -2 * q1) + 2 * Dn[0] * a * (np.exp(-a * (q1 - q7)) - 1) * np.exp(-a * (q1 - q7)) - KK * (2 * q1 - q10 - q10),
                     K * (q3 + q1 - 2 * q2) + 2 * Dn[1] * a * (np.exp(-a * (q2 - q8)) - 1) * np.exp(-a * (q2 - q8)) - KK * (2 * q2 - q11 - q11),
                     K * (q4 + q2 - 2 * q3) + 2 * Dn[2] * a * (np.exp(-a * (q3 - q9)) - 1) * np.exp(-a * (q3 - q9)) - KK * (2 * q3 - q12 - q12),
                     K * (q5 + q3 - 2 * q4) + 2 * Dn[3] * a * (np.exp(-a * (q4 - q10)) - 1) * np.exp(-a * (q4 - q10)) - KK * (2 * q4 - q7 - q7),
                     K * (q6 + q4 - 2 * q5) + 2 * Dn[4] * a * (np.exp(-a * (q5 - q11)) - 1) * np.exp(-a * (q5 - q11)) - KK * (2 * q5 - q8 - q8),
                     K * (q1 + q5 - 2 * q6) + 2 * Dn[5] * a * (np.exp(-a * (q6 - q12)) - 1) * np.exp(-a * (q6 - q12)) - KK * (2 * q6 - q9 - q9),

                     K * (q8 + q12 - 2 * q7) - 2 * Dn[0] * a * (np.exp(-a * (q1 - q7)) - 1) * np.exp( -a * (q1 - q7)) + KK *  ( q4 + q4-2 * q7),
                     K * (q9 + q7 - 2 * q8) - 2 * Dn[1] * a * (np.exp(-a * (q2 - q8)) - 1) * np.exp( -a * (q2 - q8)) + KK * (q5 + q5 - 2 * q8),
                     K * (q10 + q8 - 2 * q9) - 2 * Dn[2] * a * (np.exp(-a * (q3 - q9)) - 1) * np.exp( -a * (q3 - q9)) + KK * (q6+ q6 - 2 * q9),
                     K * (q11 + q9 - 2 * q10) - 2 * Dn[3] * a * (np.exp(-a * (q4 - q10)) - 1) * np.exp( -a * (q4 - q10)) +KK * (q1 + q1 - 2 * q10),
                     K * (q12 + q10 - 2 * q11) - 2 * Dn[4] * a * (np.exp(-a * (q5 - q11)) - 1) * np.exp( -a * (q5 - q11)) + KK * (q2 + q2 - 2 * q11),
                     K * (q7 + q11 - 2 * q12) - 2 * Dn[5] * a * (np.exp(-a * (q6 - q12)) - 1) * np.exp( -a * (q6 - q12)) + KK * (q3 + q3 - 2 * q12),
                          ]).T
